...
public class RedisUtil {
    ...
    /**
     * 模糊查询keys !!!! 生产环境禁止使用,容易造成IO堵塞,改用scan
     * @param key
     * @return
     */
    ...
    /**
     * 设置 Object 类型 value 并添加过期时间 (分秒单位)
     * @param key
     * @param value
     * @param time 过期时间,分秒单位
     */
    public void setObjectForTimeCustom(String key,Object value,long time){
    ...
    /**
     * 获取指定key 的过期时间,加上了时间单位，
     * @param key
     * @return
     */
    public Long getExpireToTimeUnit(String key){
    ...
    /**
     * 通过key前缀获取 key 下的 所有 key 字段名（key+"*"）
     * @param key
     * @return
     */
    public Set<Object> listKeys(String key) {
        return redisTemplate.keys(key);
    }
     ...
     * @return 出栈的值
     */
    public Object rightPopForTime(String key,long l, TimeUnit timeUnit){
        return redisTemplate.opsForList().rightPop(key,l,timeUnit);
    }
